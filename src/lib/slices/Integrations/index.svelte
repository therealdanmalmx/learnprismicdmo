<script>
	...
	import IconCloudflre from '~icons/fa-brands/cloudflare';
	...
	const icons = {
		...
		figma: IconFigma,
		fly: IconFly,
		cloudflare: IconCloudflre,
		digital_ocean: IconDigitalOcean,
	};
...
>
	...
	<div class="relative">
		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.integration_icons as item, index}
				{#if item.icon}
					{#if index === Math.floor(slice.primary.integration_icons.length / 2)}
						<StylizedLogoMark />
					...
					{#if index !== slice.primary.integration_icons.length - 1}
						<div
							class={clsx(
								'signal-line',
