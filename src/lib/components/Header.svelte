<script>
	...
	import IconOpen from '~icons/ph/list-bold';
	...
	>
		<div class="flex items-center justify-between">
			...
			<button
				...
				type="button"
				class="block pt-2 text-3xl text-white md:hidden"
				on:click={toggleOpen}><IconOpen /></button
			>
		</div>
					...
					<li>
						{#if item.cta_button}
							<ButtonLink
								field={item.link}
								on:click={close}
								aria-current={isActive(item.link) ? 'page' : undefined}>{item.label}</ButtonLink
							>
						{:else}
							<PrismicLink
								on:click={close}
